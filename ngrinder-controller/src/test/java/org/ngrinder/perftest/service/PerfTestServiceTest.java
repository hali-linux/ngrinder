...
public class PerfTestServiceTest extends AbstractPerfTestTransactionalTest {
	...
	public void testGetMonitorDataWithExistingData() throws IOException {
		...
		// Then
		assertFalse(listNotEmpty.test(reportDataMap.get("cpu")));
		assertFalse(listNotEmpty.test(reportDataMap.get("memory")));
		assertFalse(listNotEmpty.test(reportDataMap.get("received")));
		assertFalse(listNotEmpty.test(reportDataMap.get("sent")));
	}
